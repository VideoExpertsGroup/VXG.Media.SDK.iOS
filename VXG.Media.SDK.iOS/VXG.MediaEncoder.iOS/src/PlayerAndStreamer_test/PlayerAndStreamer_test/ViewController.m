...
- (void)viewDidLoad {
    ...
    VXG_CaptureSDK_LogLevel = LL_TRACE;
    ...
    [vxg_player_cfg setEnableInternalAudioSessionConfigure:0];

    [MediaPlayerConfig setLogLevelForObjcPart: LOG_LEVEL_NONE];
...
- (IBAction)b_stream_start:(UIButton *)sender {
    if (sender.tag == 0) {
        if ([vxg_capture_cfg getStreamType] == STREAM_TYPE_RTMP_PUBLISH) {
            [vxg_capture_cfg setRTMPurl: tf_capture.text];
        }
